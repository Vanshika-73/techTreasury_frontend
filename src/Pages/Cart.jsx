...
function Cart() {
    ...
    return (
      <Stack>
      {/* {loading && <Loading/>} */}
      {(!userInfo)&& (<Alert severity='info'>You are not logged in, Please <Link underline='always' sx={{cursor:"pointer"}} onClick={()=>navigate('/login')}>Login here</Link></Alert>)}
      {
       userInfo && (items?.length==0 ? (<Alert severity='info'>Your cart is Empty, <Link underline='always' sx={{cursor:"pointer"}} onClick={()=>navigate('/')}>Continue Exploring</Link></Alert>) : 
       (
       <Grid container>
        <Grid item md={8}>
        <List>
            ...
            <List>
              <ListItem>
                <Typography variant='h6'>
                  ...
                  Total Amount: <CurrencyRupeeIcon sx={{mt:1.7}}/>{amount && amount.toFixed(2)}
                </Typography>
              </ListItem>
              <Divider/>
              <ListItem>
